    ...
    <Layout>
      ...
      <section className="container-x pt-8">
        ...
        <nav aria-label="Argomenti" className="mt-8 flex flex-wrap gap-2.5">
          {faqTopics.map((t) => (
            <a key={t.key} href={`#${t.key}`} className="rounded-full border border-line bg-white px-4 py-2 text-sm text-ink-soft transition-colors hover:border-brass hover:text-brass-dark">
              {t.label}
            </a>
          ))}
        </nav>
